...
class RestaurantManagementSystem:
    ...
    def show_bill_popup(self):
        ...
        items_str = ', '.join([f"{item} x {quantity}" for item, quantity in selected_items])
        self.cursor.execute('INSERT INTO past_records (customer_name, customer_contact, items, total_price) VALUES (?, ?, ?, ?)',
                            (self.customer_name.get(), self.customer_contact.get(), items_str, total_price + gst_amount))
        self.conn.commit()
    ...
    def past_records(self):
        ...
        self.cursor.execute('SELECT * FROM past_records')
        records = self.cursor.fetchall()

        records_text = tk.Text(records_window, height=10)
        records_text.pack(fill="x", padx=10, pady=10)

        for record in records:
            record_str = f"Customer Name: {record[1]}\n"
            record_str += f"Customer Contact: {record[2]}\n"
            record_str += f"Items: {record[3]}\n"
            record_str += f"Total Price: {self.convert_to_inr(record[4])}\n\n"
